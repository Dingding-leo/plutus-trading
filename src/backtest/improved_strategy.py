...
class ImprovedStrategy:
    ...
    def _can_trade_symbol(self, symbol: str) -> bool:
        """
        Enforce BTC > ETH > ALT rule from TRADING_WORKFLOW.md.
        - BTC: Always allowed
        - ETH: Only if BTC is not weak
        - ALT: Only if BTC is strong
        """
        if symbol == "BTC-USDT":
            return True
        elif symbol == "ETH-USDT":
            return self.market_context.get("btc_strength") != "weakness"
        else:
            # ALT coins: only if BTC is strong
            return self.market_context.get("btc_strength") == "strength"

    def calculate_risk_level(self, candles: List[dict]) -> str:
        """
        Calculate risk level based on ATR and market conditions.
        HIGH: ATR >= 1.5x average (high volatility)
        ...
        try:
            ...
            avg_atr_list = []
            for i in range(14, min(50, len(candles)-14)):
                sub_closes = closes[:i]
                sub_highs = highs[:i]
                sub_lows = lows[:i]
                avg_atr_list.append(indicators.calculate_atr(sub_highs, sub_lows, sub_closes, 14))

            avg_atr = sum(avg_atr_list) / len(avg_atr_list) if avg_atr_list else atr

            # Current ATR as percentage of price
            current_atr_pct = (atr / closes[-1]) * 100
            avg_atr_pct = (avg_atr / closes[-1]) * 100

            # HIGH risk if ATR is 1.5x or more than average
            if current_atr_pct >= avg_atr_pct * 1.5:
                return "HIGH"
            elif current_atr_pct >= avg_atr_pct * 1.25:
                return "MODERATE"
            else:
                return "LOW"
    ...
    def check_entry(
        ...
        # BTC > ETH > ALT enforcement (CRITICAL from TRADING_WORKFLOW.md)
    ...
    def execute(
        ...
        # Update dynamic risk level based on ATR
        if analysis.get("candles"):
            self.market_context["risk_level"] = self.calculate_risk_level(analysis["candles"])
